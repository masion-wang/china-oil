...
define(function (require) {
  ...
  Vue.gvUtil.setApi({
    ...
    // 详情接口
    findBillOut: "/gppayment/bringOutBill",
    // 分页查询2 新增单据
    addGpPayment: "/gppayment/addGpPayment",
    // 新增接口
    findfeetypecode: "/guFeetype/findfeetypecode",
    // 单据类型
    offsetCalculation: "/gppayment/offsetCalculation",
    // 计算轧差
    policySelfMainpagetWorkNext: "/gppayment/getWorkNext",
    //工作流弹框
    gppaymentsubmit: "/gppayment/submit",
    // 提交
    find_other_list: "/gg_code/find_other_list",
    // 账单人名称
    getUserInfo: "/common/searchInfo",
    // 获取当前用户信息
    showPaymentMark: "/gppayment/showPaymentMark",
    // 判断付款还是收款
  ...
  return Vue.gvUtil.Page({
    ...
    created: function created() {
      ...
      }); // let url2 = 'system/gg_code/find_other_list'
      //  Vue.gvUtil.getUrl({
      //   apiName: "find_other_list",
      //   contextName: 'system',
      // });
      // let params2 = {
      //   poName: "ggSupplierMsg"
      // }
      // Vue.gvUtil.http.post(url2, params2).then((res) => {
      //   console.log('res', res)
      //   if (res.resCode == '0000') {
      //     console.log('账单人名称', res)
      //     that.correspondenceList = res.resData.businessList
      //   }
      // });

      var url3 = "product/common/searchInfo";
      Vue.gvUtil.http.get(url3).then(function (res) {
        if (res.resCode == "0000") {
          console.log("集团个人信息", res.resData.userInfo.companyName); // 这是个数组

          var companyName = res.resData.userInfo.companyName;
          ...
          Vue.gvUtil.http.post(url4, params3).then(function (res) {
            console.log("res", res);
    ...
    methods: {
      ...
      onListBtn: function onListBtn(row, type) {
        ...
        var paymentMark = row.paymentMark;
        var applicationNo = row.applicationNo;
      ...
      // 查看付款 收款
      getlookdata: function getlookdata(applicationNo, paymentMark) {
        var _this = this;
        ...
        Vue.gvUtil.http.post(url, params).then(function (res) {
          ...
          if (res.resCode == "0000") {
            if (paymentMark == "01") {
              ...
              that.baseInfo122.baseInfoRule = _baseInfoRule;
            }
          } else {
            _this.$message({
              showClose: true,
        ...
        Vue.gvUtil.http.post(url, params).then(function (res) {
          if (res.resCode === "0000") {
            if (res.resData.length != 0) {
              //工作流弹框
              _this2.workflowdialog = true;

              if (res.resData[0].mapList) {
                _this2.gwNextNodeExecutorsList = res.resData[0].mapList;
              } else {
                _this2.gwNextNodeExecutorsList = res.resData;
              }
      ...
      calfukuan: function calfukuan() {
        ...
        Vue.gvUtil.http.post(url, params).then(function (res) {
          ...
          if (res.resCode == "0000") {
            ...
            that.baseInfo1.baseInfoRule = baseInfoRule; // 弹出弹窗 获取后台返回单据类型
      ...
      // 收款计算轧差
      calshoukuan: function calshoukuan() {
        var that = this;
        ...
        Vue.gvUtil.http.post(url, params).then(function (res) {
          ...
          if (res.resCode == "0000") {
            ...
            that.baseInfo12.baseInfoRule = baseInfoRule; // 弹出弹窗 获取后台返回单据类型
      ...
      openpaymentadd: function openpaymentadd() {
        ...
        if (
          ...
          this.$message({
            message: "分入/分出 保费/赔费 不能为空",
            type: "warning",
          });
          return;
        } // if (this.baseInfo1.dueDatesStart == '' && this.baseInfo1.dueDatesEnd == "") {
        //   this.$message({
        //     message: "缴费截止起止日期不能全为空",
        //     type: "warning",
        //   });
        //   return
        // }
      ...
      // 收款申请新增弹出页面
      openpaymentadd22: function openpaymentadd22() {
        ...
        if (
          ...
          return;
        } // if (this.baseInfo12.dueDatesStart == '' && this.baseInfo12.dueDatesEnd == "") {
        //   this.$message({
        //     message: "缴费截止起止日期不能全为空",
        //     type: "warning",
        //   });
      ...
      // 添加 收款
      addData2: function addData2() {
        var a = this.$refs.table4.getSelectData();
        console.log("添加 a", a);
        ...
        try {
          for (
            var _iterator2 = a[Symbol.iterator](), _step2;
      ...
      // 申请状态-shoukuan
      changeschemeName3: function changeschemeName3(e, row, index) {
        console.log("看看", e, row, index);
        var that = this;
        row.EP = e.codeCode;
      ...
      // 收款确认弹出页面
      opengetment: function opengetment() {
        this.isShow3 = true;
        this.paymentMark = "00";
      },
      // 付款移除
      removefukuan: function removefukuan(index) {
        this.guPolicyVoListfukuan.splice(index, 1);
      },
      // 收款移除
      removeshoukuan: function removeshoukuan(index) {
      ...
      // 付款 剩余金额
      getrestMoneyfukuan: function getrestMoneyfukuan(e, row, index) {
        console.log("看看", e, row, index);
        var that = this;
      ...
      // 收款 剩余金额
      getrestMoneyshoukuan: function getrestMoneyshoukuan(e, row, index) {
        console.log("看看", e, row, index);
        var that = this;
        row.availAmount = row.outstanding - e;
        this.$set(that.guPolicyVoListshoukuan, index, row);
