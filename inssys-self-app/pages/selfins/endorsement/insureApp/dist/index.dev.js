...
define(function (require) {
  ...
  return Vue.gvUtil.Page({
    ...
    mounted: function mounted() {
      console.log('审核人员选中返回来的数据', this.$route.query); // 审核

      if (this.$route.query.flag == "Approve") {
        var proposalNo = this.$route.query.row[0].proposalNo;
        var versionNo = this.$route.query.row[0].versionNo;
        var policyNo = this.$route.query.row.policyNo;
        var policyMainId = this.$route.query.row[0].policyMainId;
        this.status2 = 'Approve';
        this.$refs.baseInfo.getPolicyFeeInfo(proposalNo, versionNo, 'Approve', policyNo, policyMainId);
      } // 修改 临时
      else if (this.$route.query.flag == "modify") {
          console.log('跳转的修改');
          var _proposalNo = this.$route.query.row[0].proposalNo;
          var _versionNo = this.$route.query.row[0].versionNo;
          var _policyNo = this.$route.query.row.policyNo;
          var _policyMainId = this.$route.query.row[0].policyMainId;
          this.status2 = 'modify';
          this.$refs.baseInfo.getPolicyFeeInfo2(_proposalNo, _versionNo, "modify", _policyNo, _policyMainId);
        } // 控制台(工作流修改) 修改
            ...
            console.log('obj', obj);
            this.status2 = 'modify';
            this.$refs.baseInfo.getPolicyFeeInfo22(obj.proposalNo, obj.versionNo, "modify", obj.policyMainId);
          } // 控制台(工作流修改) 审核
              ...
              this.status2 = 'Approve';
              this.$refs.baseInfo.getPolicyFeeInfo11(_obj.proposalNo, _obj.versionNo, "Approve", _obj.policyMainId);
            } // 控制台(工作流修改) 查看
                ...
                var _obj2 = JSON.parse(this.taskObj.param1);

                console.log('obj', _obj2);
                this.status2 = 'Look';
                this.$refs.baseInfo.getPolicyFeeInfo44(_obj2.proposalNo, _obj2.versionNo, "Look", _obj2.policyMainId);
              } // 查看
              else if (this.$route.query.flag == "Look") {
                  console.log('跳转的修改');
                  var _proposalNo2 = this.$route.query.row.proposalNo;
                  var _versionNo2 = this.$route.query.row.versionNo;
                  var _policyNo2 = this.$route.query.row.policyNo;
                  var _policyMainId2 = this.$route.query.row.policyMainId;
                  this.status2 = 'Look';
                  this.$refs.baseInfo.getPolicyFeeInfo3(_proposalNo2, _versionNo2, 'Look', _policyNo2, _policyMainId2);
                } else {
                  this.status2 = 'modify';
                }
    },
    ...
    methods: {
      ...
      clear: function clear() {
        ...
        this.$nextTick(function () {
          _this2.$refs.selectpolicyApp.clear();
        });
      },
      // 打开查询弹窗
      getDialogShow: function getDialogShow(isShow, vailfFourBrother, riskCode, guPolicyVoList) {
        ...
        this.$nextTick(function () {
          console.log('this.$refs', _this3.$refs);

          _this3.$refs.selectpolicyApp.getValidFourBrother();
      ...
      save: function save() {
        ...
        params2.insuredCode = params2.insured; // // debugger
        ...
        if (this.valid == true) {
          // 维护期不能为空 maintenancePeriods
          if (params2.guPolicyVoList[0].cedingRiskCode == 'O-OCI' || params2.guPolicyVoList[0].cedingRiskCode == 'O-OCL' || params2.guPolicyVoList[0].cedingRiskCode == 'E-CIE' || params2.guPolicyVoList[0].cedingRiskCode == 'E-CAR') {
            if (params2.maintenancePeriods.length != 2) {
              this.$message({
                showClose: true,
                message: '维护期不能为空',
                type: 'warn'
        ...
        Vue.gvUtil.http.post(url, this.taskObj).then(function (res) {
          if (res.resCode === "0000") {
            if (res.resData.length != 0) {
              //工作流弹框
              _this5.workflowdialog = true;
              _this5.gwNextNodeExecutorsList = res.resData;
      ...
      submit: function submit() {
        ...
        if (this.valid == true) {
          ...
          var url = 'selfins/policySelfMain/submit'; //  Vue.gvUtil.getUrl({
          //   apiName: "submit",
          //   contextName: "selfins",
          // });
          ...
          Vue.gvUtil.http.post(url, params2).then(function (res) {
            ...
            if (res.resCode == "0000") {
              ...
              _this6.$alert(res.resData.proposalNo, '投保单号', {
                confirmButtonText: '确定',
                callback: function callback(action) {
                  console.log('跳转保批单查询'); // 工作台
