  ...
  return Vue.gvUtil.Page({
    ...
    methods: {
      ...
      add: function add(obj) {
        ...
        var randomarr = this.getRandomOnly(); // 原保单

        guPolicyVoList.push(obj);
        objFromSelectpolicyApp.guPolicyVoList = guPolicyVoList; // 标的

        objFromSelectpolicyApp.guPolicyItemMainVoList = this.tablearr[index].guPolicyItemMainList; // 再保人

        objFromSelectpolicyApp.guPolicyRiVoList = this.tablearr[index].guPolicyRiVoList; // 原保单增加唯一标识

        objFromSelectpolicyApp.guPolicyVoList[0].random = randomarr; // 标的增加唯一标识
        ...
        try {
          for (var _iterator = objFromSelectpolicyApp.guPolicyItemMainVoList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            var son = _step.value;
            son.random = randomarr;
          } // 再保人增加唯一标识
        ...
        try {
          for (var _iterator2 = objFromSelectpolicyApp.guPolicyRiVoList[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            var son2 = _step2.value;
            son2.random = randomarr;
          }
        } catch (err) {
        ...
        console.log('objFromSelectpolicyApp22', objFromSelectpolicyApp); // 校验四兄弟
        ...
        for (var _i = 0, _guPolicyVoList = guPolicyVoList; _i < _guPolicyVoList.length; _i++) {
          ...
          try {
            for (var _iterator3 = this.guPolicyVoList2[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
...
              });
        ...
        if (validF.cedingCompany == null && validF.currency == null && validF.programmeCode == null && validF.projectCode == null) {} else {
...
            });
      ...
      handleSelectionChange: function handleSelectionChange(val) {
        ...
        for (var i = 0; i < val.length; i++) {
          ...
          console.log('多选标的', this.tablearr[index].guPolicyItemMainList); // 生成保单唯一标识

          var randomarr = this.getRandomOnly(); // 原单

          this.tablearr[index].guPolicy.random = randomarr; // 标的
          ...
          try {
            for (var _iterator5 = item2[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
              var son2 = _step5.value;
              son2.random = randomarr;
      ...
      multiAdd: function multiAdd() {
        if (this.objFromSelectpolicyApp.guPolicyVoList && this.objFromSelectpolicyApp.guPolicyVoList.length > 0) {
          ...
          try {
            for (var _iterator6 = this.objFromSelectpolicyApp.guPolicyVoList[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
              ...
              try {
                for (var _iterator8 = this.guPolicyVoList2[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
                  var item2 = _step8.value;
...
              });
          ...
          if (validF.cedingCompany == null && validF.currency == null && validF.programmeCode == null && validF.projectCode == null) {
            // 第一次用户多选的时候也必须满足
...
            });
              ...
              try {
                for (var _iterator7 = this.objFromSelectpolicyApp.guPolicyVoList[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
...
            });
      ...
      cmp: function cmp(x, y) {
        var tmpObjx = JSON.stringify(x);
        var tmpObjy = JSON.stringify(y);
        var tmpObjx1 = JSON.parse(tmpObjx);
        var tmpObjy1 = JSON.parse(tmpObjy); // let xtrr = tmpObjx1.index
        // let ytrr = tmpObjy1.index
        // let xpolicyIdtrr = tmpObjx1.policyId
        // let ypolicyIdtrr = y1.policyId

        delete tmpObjx1.policyId;
        delete tmpObjx1.policyMainId;
        delete tmpObjx1.periodEnd;
        delete tmpObjx1.supplierShortName;
        delete tmpObjx1.randomValue;
        tmpObjx1.index = 1;
        delete tmpObjy1.policyId;
        delete tmpObjy1.policyMainId;
        delete tmpObjy1.periodEnd;
        delete tmpObjy1.supplierShortName;
        delete tmpObjy1.randomValue;
        tmpObjy1.index = 1; // delete y.index

        console.log('tmpObjx1 tmpObjy1 ', tmpObjx1, tmpObjx1); // If both x and y are null or undefined and exactly the same 

        if (tmpObjx1 === tmpObjy1) {
          return false;
        } // If they are not strictly equal, they both need to be Objects 


        if (!(tmpObjx1 instanceof Object) || !(tmpObjy1 instanceof Object)) {
          return false;
        } //They must have the exact same prototype chain,the closest we can do is
        //test the constructor. 


        if (tmpObjx1.constructor !== tmpObjy1.constructor) {
          return false;
        }

        for (var p in tmpObjx1) {
          // 如果属性名为 policyId policyMainId 直接跳过
          //Inherited properties were tested using x.constructor === y.constructor
          if (tmpObjx1.hasOwnProperty(p)) {
            // Allows comparing x[ p ] and y[ p ] when set to undefined 
            if (!tmpObjy1.hasOwnProperty(p)) {
              return false;
            } // If they have the same strict value or identity then they are equal 


            if (tmpObjx1[p] === tmpObjy1[p]) {
              continue;
            } // Numbers, Strings, Functions, Booleans must be strictly equal 


            if (_typeof(tmpObjx1[p]) !== "object") {
              return false;
        ...
        for (p in tmpObjy1) {
          ...
          if (tmpObjy1.hasOwnProperty(p) && !tmpObjx1.hasOwnProperty(p)) {
            return false;
          }
        } // x.index = xtrr
        // y.index = ytrr
        // x.policyId = xpolicyIdtrr
        // y.policyId = ypolicyIdtrr
