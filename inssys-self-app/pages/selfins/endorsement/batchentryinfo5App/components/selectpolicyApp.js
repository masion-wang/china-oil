  ...
  return Vue.gvUtil.Page({
    ...
    methods: {
      ...
      add(obj) {
        ...
        let randomarr = this.getRandomOnly()
        // 原保单
        ...
        // 原保单增加唯一标识
        objFromSelectpolicyApp.guPolicyVoList[0].random = randomarr
        // 标的增加唯一标识
        for (let son of objFromSelectpolicyApp.guPolicyItemMainVoList) {
          son.random = randomarr
        }
        // 再保人增加唯一标识
        for (let son2 of objFromSelectpolicyApp.guPolicyRiVoList) {
          son2.random = randomarr
        }
        console.log('objFromSelectpolicyApp22', objFromSelectpolicyApp)
        // 校验四兄弟
...
              });
        ...
        if (validF.cedingCompany == null &&
          validF.currency == null &&
          validF.programmeCode == null &&
          validF.projectCode == null) {} else {
...
            });
      ...
      handleSelectionChange(val) {
        ...
        for (let i = 0; i < val.length; i++) {
          ...
          let randomarr = this.getRandomOnly()
          // 原单
          this.tablearr[index].guPolicy.random = randomarr
          // 标的
...
            });
      ...
      // 去重判断
      cmp(x, y) {
        let tmpObjx = JSON.stringify(x);
        let tmpObjy = JSON.stringify(y);
        let tmpObjx1 = JSON.parse(tmpObjx);
        let tmpObjy1 = JSON.parse(tmpObjy);
        // let xtrr = tmpObjx1.index
        ...
        tmpObjy1.index = 1
        // delete y.index
        console.log('tmpObjx1 tmpObjy1 ', tmpObjx1, tmpObjx1)
