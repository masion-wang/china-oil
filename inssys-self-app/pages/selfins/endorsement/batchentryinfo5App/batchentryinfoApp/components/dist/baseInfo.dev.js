...
define(function (require) {
  ...
  return Vue.gvUtil.Page({
    ...
    methods: {
      ...
      getPolicyFeeInfo3: function getPolicyFeeInfo3(proposalNo, versionNo, word, policyNo) {
        ...
        Vue.gvUtil.http.post(url, params).then(function (res) {
          if (res.resCode == '0000') {
            ...
            _this3.$emit('fromChild', res.resData.guPolicyAllInfo.ggDocumentList);
      ...
      // 保单详情接口-审核
      getPolicyFeeInfo: function getPolicyFeeInfo(proposalNo, versionNo, word, policyNo) {
        var _this4 = this;
      ...
      givedata: function givedata(obj) {
        ...
        this.baseInfo.policyStatus = obj.policyStatus; // 如果obj有submit 且 为true
      ...
      importExcel: function importExcel(file) {
        ...
        if (that.baseInfo.riskCode != 'EAS' && that.baseInfo.riskCode != 'MAR' && that.baseInfo.riskCode != 'OOPU') {
          this.$confirm('当前保单的标的详情信息已由原保单带入，是否确认要通过导入清单导入？', '提示', {
            ...
            Vue.gvUtil.http.post(url, formData).then(function (res) {
              ...
              if (res.resCode != '0000') {
                ...
                _this6.$message.success('导入失败');

                return;
              }

              if (res.resData[0].riskCode != that.baseInfo.riskCode) {
                _this6.$message.success('请勿导入不同险种的标的');

                return;
              }

              _this6.$message.success('导入成功！'); // 是这五个险种
          ...
          Vue.gvUtil.http.post(url, formData).then(function (res) {
            ...
            _this6.$message.success('导入成功！'); // 传递premiumList 接口 ？？？？
      ...
      importClauseExcel: function importClauseExcel(file) {
        ...
        Vue.gvUtil.http.post(url, formData).then(function (res) {
          ...
          _this7.$message.success('导入成功！'); // console.log('that.underlying.guPolicyItemMainVoList ',that.underlying.guPolicyItemMainVoList )
          // that.underlying.guPolicyItemMainVoList = res.resData
