...
define(function (require) {
    ...
    var SelectTopic = require('../../messageConfigApp/index');
    var config = {
        api: {
            ...
            'sysGetMessage': '/gg_message/getByMessageCode/{messageCode}',
        },
    ...
    return Vue.gvUtil.Page({
        ...
        events: {
            ...
            addEmail(){
                this.editEmails.push({userName:'',email:''})
            ...
            confirmEmail(){
                ...
                this.emails = []
                    ...
                    for (var i = 0; i < this.editEmails.length;i++){
                        this.emails.push(this.editEmails[i].userName)
                        let obj = {}
        ...
        methods: {
            ...
            //     }
                ...
                if (this.dialogProp.type=="001"){
                    this.value = "各位计划编制岗同事：<br/> &nbsp&nbsp&nbsp&nbsp" + this.dialogProp.name + "项目初始化已经完成，代码为：" + this.embedCode + ",请各供应商同事及时出单" + "<br/> <br/>" + "谢谢！祝身体健康，工作顺利<br/>中海油项目编制处",
                    this.topic = "项目编制完成通知"
                } else if (this.dialogProp.type == "002"){
                    this.value = "各位供应商同事：<br/> &nbsp&nbsp&nbsp&nbsp" + this.dialogProp.name + "出单通知已经发出，代码为：" + this.embedCode+",请各供应商同事及时出单" + "<br/> <br/>" + "谢谢！祝身体健康，工作顺利<br/>中海油项目编制处",
                    this.topic = "出单通知"
                }else{
                    this.value = "各位供应商同事：<br/> &nbsp&nbsp&nbsp&nbsp" + this.dialogProp.name + "出险通知已经发出，代码为：" + this.embedCode + ",请各供应商同事知悉" + "<br/> <br/>" + "谢谢！祝身体健康，工作顺利<br/>中海油项目编制处",
                    this.topic = "出险通知"
                }
            ...
            getMessageCode(code){
                debugger
                this.selectEmailDialog = false
                this.emailAddress = []
                this.emails = []
                var _this = this,
                    url = Vue.gvUtil.getUrl({
                        apiName: 'sysGetMessage',
                        contextName: 'auth',
                        urlParams: {
                            messageCode: code
                        }
                ...
                Vue.gvUtil.http.post(url).then(function (res) {
                    if (res.resCode === '0000') {
                        _this.topic = res.resData.gtMessageVo.titleValue
                        _this.content = res.resData.gtMessageVo.contentValue.replace("#code", _this.embedCode)
                        let arr = res.resData.gtMessageVo.sysUserQueryResVoList
                   

                        for(var i=0;i<arr.length;i++){
                            _this.$set(_this.emails,i,arr[i].userName)
                            let obj = {}
