    ...
    return Vue.gvUtil.Page({
        ...
        events: {
            onUserInquery: function (row) {
                ...
                Vue.gvUtil.registerConfigExtend('taskAssignQueryApp', function () {
                    require.async('./pages/taskAssignQuery', function (taskTransferInqueryAPP) {
            ...
            onSave: function () {
                ...
                // 处理中的任务移交
                if (_this.status !== 'Processed') {
                    Vue.gvUtil.http.get(url).then(function (res) {
                        if (res.resCode === '0000' && res.resData === '0') {
                            Vue.gvUtil.alert({msg: Vue.gvUtil.getInzTranslate('sysTaskAssignSuccess')});
                            _this.onGetList();
                        } else {
                            Vue.gvUtil.message(Vue.gvUtil.getInzTranslate('sysTaskAssignFailure'));
                        }
                    });
                } else {
                    Vue.gvUtil.message(Vue.gvUtil.getInzTranslate('sysTaskAssignNotProcessed'))
                }
                this.dialogForm.openDialog = !this.dialogForm.openDialog
            },
            // 获取功能列表
            onGetList: function () {
                ...
                this.taskTransferAppVoFilters['isTaskAssign'] = true;
                var _this = this;
